    ...
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/40 backdrop-blur-xs flex justify-end animate-in fade-in duration-150">
        ...
        <div className="p-5 border-b border-slate-200 bg-slate-50/70 flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                ...
                {liveStudent.schoolCode && liveStudent.schoolCode !== 'NONE' ? (
                  ...
                  <span className="px-2 py-0.5 rounded-full text-[11px] font-semibold bg-slate-100 text-slate-700">
                    K{String(liveStudent.birthYear).slice(-2)}
                  </span>
                )}
              </div>
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-xl text-xs font-bold text-amber-800 bg-amber-50 hover:bg-amber-100 border border-amber-200 transition-colors shadow-2xs cursor-pointer"
              ...
              <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200/80 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() => setShowEditModal(true)}
                    className="inline-flex items-center space-x-1 px-2.5 py-1 text-[11px] font-bold text-blue-700 bg-white hover:bg-blue-50 border border-blue-200 rounded-lg transition-colors shadow-2xs cursor-pointer"
                  >
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div>
                    ...
                    <p className="font-semibold text-slate-800">{liveStudent.fullName}</p>
                  </div>
                  <div>
                    <span className="text-slate-500">Ngày sinh:</span>
                    <p className="font-semibold text-slate-800">
                      {liveStudent.dob} (Khóa K{String(liveStudent.birthYear).slice(-2)})
                    </p>
                  </div>
                  <div>
                    <span className="text-slate-500">Số điện thoại HS:</span>
                    <p className="font-semibold text-slate-800">{liveStudent.phone || 'Chưa có'}</p>
                  </div>
                  <div>
                    <span className="text-slate-500">Email học sinh:</span>
                    <p className="font-semibold text-slate-800">{liveStudent.email || 'Chưa cập nhật'}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-slate-800">
                      {liveStudent.schoolGrade ? `Lớp ${liveStudent.schoolGrade}` : 'Chưa cập nhật'}
                    </p>
                  </div>
                  <div>
                    <span className="text-slate-500">Trạng thái theo học:</span>
                    <div className="pt-0.5">
                      <span
                        className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-bold ${
                          liveStudent.status === 'active'
                            ? 'bg-emerald-100 text-emerald-800'
                            : liveStudent.status === 'paused'
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-slate-800">{liveStudent.joinedDate || '2026-06-01'}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200/80 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() => setShowEditModal(true)}
                    className="inline-flex items-center space-x-1 px-2.5 py-1 text-[11px] font-bold text-emerald-700 bg-white hover:bg-emerald-50 border border-emerald-200 rounded-lg transition-colors shadow-2xs cursor-pointer"
                  >
                    <Edit2 className="w-3 h-3" />
                    <span>Sửa liên hệ PH</span>
                  </button>
                </div>
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div>
                    ...
                    <p className="font-semibold text-slate-800">
                      {liveStudent.parentName || <span className="italic text-slate-400">Chưa cập nhật</span>}
                    </p>
                  </div>
                  <div>
                    <span className="text-slate-500">SĐT Phụ huynh:</span>
                    <p className="font-semibold text-slate-800">
                      {liveStudent.parentPhone || <span className="italic text-slate-400">Chưa cập nhật</span>}
                    </p>
                  </div>
                  <div className="col-span-2">
                    <span className="text-slate-500">Email Phụ huynh:</span>
                    <p className="font-semibold text-slate-800">
                      {liveStudent.parentEmail || 'Chưa cập nhật'}
                    </p>
                  </div>
                </div>
              </div>
      ...
      {showEditModal && (
        <EditStudentModal
          ...
          student={liveStudent}
          onSuccess={(updated) => {
            showToast(`Đã cập nhật thông tin học sinh ${updated.fullName} thành công!`);
          }}
        />
      )}
    </div>
